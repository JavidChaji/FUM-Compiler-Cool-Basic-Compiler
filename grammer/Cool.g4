...
classDefine
   : CLASS className = TYPEID (INHERITS parentClass = TYPEID)? '{' (feature ';')* '}'
   ;

feature
   : methodName = OBJECTID '(' atribute ')' ':' returnType = TYPEID '{' expression '}' # method
   | propertyName = OBJECTID ':' propertyType = TYPEID (ASSIGNMENT expression)? # property
   ;
...
formal
   : formalName = OBJECTID ':' formalType = TYPEID
   ;
...
expression
   ...
   | '{' (expression ';')+ '}' # block
   | LET letName = OBJECTID ':' letType = TYPEID (ASSIGNMENT expression)? (',' OBJECTID ':' TYPEID (ASSIGNMENT expression)?)* IN expression # letIn
